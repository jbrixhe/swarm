...
impl EntityAllocator {
    ...
    pub fn create_entities(&mut self) -> CreateEntityIter {
        CreateEntityIter {
    ...
    pub(crate) fn delete_all_entities(&mut self) {
        for block in self.blocks.blocks.drain(..) {
...
impl<'a> Iterator for CreateEntityIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut block = self.allocator.allocate();
